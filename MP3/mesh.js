...
class TriMesh {
  ...
  constructor() {
    this.isLoaded = false;
    this.minXYZ = [Infinity, Infinity, Infinity];
    this.maxXYZ = [-Infinity, -Infinity, -Infinity];
  ...
  */
  computeAABB() {
    for (var i = 0; i < this.vBuffer.length; i += 3) {
      if (this.vBuffer[i] > this.maxXYZ[0]) this.maxXYZ[0] = this.vBuffer[i];
      if (this.vBuffer[i + 1] > this.maxXYZ[1]) this.maxXYZ[1] = this.vBuffer[i + 1];
      if (this.vBuffer[i + 2] > this.maxXYZ[2]) this.maxXYZ[2] = this.vBuffer[i + 2];
      if (this.vBuffer[i] < this.minXYZ[0]) this.minXYZ[0] = this.vBuffer[i];
      if (this.vBuffer[i + 1] < this.minXYZ[1]) this.minXYZ[1] = this.vBuffer[i + 1];
      if (this.vBuffer[i + 2] < this.minXYZ[2]) this.minXYZ[2] = this.vBuffer[i + 2];
    }
