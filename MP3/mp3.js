...
//----------------------------------------------------------------------------------
/**
 * Sends projection/modelview matrices to shader for skybox
 */
...
function setupShaders() {
  ...
  shaderProgram.uniformModeLoc = gl.getUniformLocation(shaderProgram, "uMode");
}
...
//-------------------------------------------------------------------------
/**
 * Sends render mode to the shader
 * @param {int} mode Current rendering mode
 */
function setModeUniform(mode) {
  gl.uniform1i(shaderProgram.uniformModeLoc, mode);
}
...
function animate() {
  ...
  if (eulerX < -180) eulerX += 360;
  if (eulerY > 180) eulerY -= 360;
  if (eulerY < -180) eulerY += 360;
  if (meshY > 180) meshY -= 360;
  if (meshY < -180) meshY += 360;

  document.getElementById("mY").value = meshY;
